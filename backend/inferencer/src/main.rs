...
pub extern "C" fn infer(tensor_ptr: i32, tensor_len: i32, result_ptr: i32) -> i32 {
    ...
    let (class_idx, confidence) = match model.run_inference_compat(tensor) {
    ...
    // Create JSON response with new structure
    let response = serde_json::json!({
        "output": get_class_label(class_idx as usize),
        "metadata": {
