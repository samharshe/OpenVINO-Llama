...
pub extern "C" fn infer_with_model(data_ptr: i32, data_len: i32, result_ptr: i32, model_id: i32) -> i32 {
    ...
    // Run inference
    let result = match model.infer(data) {
    ...
    let response = serde_json::json!({
        "output": get_class_label(result["class_index"].as_u64().unwrap_or(0) as usize),
        "metadata": {
            "probability": result["confidence"].as_f64().unwrap_or(0.0),
            "class_index": result["class_index"].as_u64().unwrap_or(0)
        },
        "model_info": {
            "name": metadata.name,
